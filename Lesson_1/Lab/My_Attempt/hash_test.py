...
def testConstructor(table):
    ...
    try:
        ...
        assert hasattr(table, "table"), title + color.YELLOW +  errMessage + color.END
        ...
        assert isinstance(table.table, list) and len(table.table) == 26,title + color.YELLOW +  errMessage + color.END
        print(title + color.GREEN + "Passed" + color.END)
        return True
...
def testInsert(table):
    ...
    try:
        ...
        assert hasattr(jeff, "key") and hasattr(jeff, "value"), title + color.YELLOW +  errMessage + color.END
        assert jeff.key == "Jeff" and jeff.value == 80, title + color.YELLOW +  errMessage + color.END
        print(title + color.GREEN + "Passed" + color.END)

        title = "Test 2: Checking for Darren | "
        errMessage = title + color.YELLOW + "Error: Indexing into Darren does not return HashItem('Darren', 82). Did you replace Daniel?" + color.END
...
def testSearch(table):
    ...
    try:
        ...
        errMessage = title + color.YELLOW + "Error: Searching Charles does not return HashItem('Charles', 72)" + color.END
        charles = table.search("Charles")
        assert hasattr(charles, "key") and hasattr(charles, "value") and  charles.key == "Charles" and charles.value == 72, errMessage
        print(title + color.GREEN + "Passed" + color.END)

        title = "Test 2: Searching Daniel | "
        errMessage = title + color.YELLOW + "Error: Searching Daniel does not return None" + color.END
