...
def testConstructor(table):
    ...
    try:
        ...
        assert hasattr(table, "table") and hasattr(table, "size"), title + getErrorColors(errMessage)
        ...
        assert isinstance(table.table, list) and len(table.table) == 5,title + color.YELLOW +  errMessage + color.END
...
Esther  | 82\n''' + color.END)
    ...
    try:
        ...
        assert hasattr(brandon, "key") and hasattr(brandon, "value"), title + color.YELLOW +  errMessage + color.END
        assert brandon.key == "Brandon" and brandon.value == 72, title + color.YELLOW +  errMessage + color.END
        print(title + color.GREEN + "Passed" + color.END)

        title = "Test 2: Checking for Esther | "
        errMessage = title + color.YELLOW + "Error: Indexing into Esther (index 3) does not return HashItem('Esther', 82)" + color.END
        ...
        errMessage = title + color.YELLOW + "Error: Table length (capacity) is not 10" + color.END
        assert len(table.table) == 10, errMessage
        print(title + color.GREEN + "Passed" + color.END)

        title = "Test 5: Checking for Kevin at index 3 (should have rehashed all items) | "
        errMessage = title + color.YELLOW + "Error: Indexing into Kevin (index 3) does not return HashItem('Kevin', 92). Did you perform a rehash of items?" + color.END
...
def testSearch(table):
    ...
    try:
        ...
        errMessage = title + color.YELLOW + "Error: Searching Esther does not return HashItem('Esther', 82)" + color.END
        esther = table.search("Esther")
        
        assert hasattr(esther, "key") and hasattr(esther, "value") and  esther.key == "Esther" and esther.value == 82, errMessage
        print(title + color.GREEN + "Passed" + color.END)

        title = "Test 2: Searching Jenn | "
        errMessage = title + color.YELLOW + "Error: Searching Jenn does not return None" + color.END
        jenn = table.search("Jenn")
        assert jenn == None, errMessage
        print(title + color.GREEN + "Passed" + color.END)

        title = "Test 3: Searching Kevin | "
        errMessage = title + color.YELLOW + "Error: Searching Kevin does not return HashItem('Kevin', 92)" + color.END
        kevin = table.search("Kevin")
        assert hasattr(kevin, "key") and hasattr(kevin, "value") and  kevin.key == "Kevin" and kevin.value == 92, errMessage
        print(title + color.GREEN + "Passed" + color.END)
        return True
...
def testDelete(table):
    ...
    try:
        ...
        kevin = table.search("Kevin")
        errMessage = title + color.YELLOW + "Error: Searching Kevin does not return None" + color.END
        assert kevin == None, errMessage
        
        errMessage = title + color.YELLOW + f"Error: Table size should be 5, got {table.size}" + color.END
        assert table.size == 5, errMessage
        print(title + color.GREEN + "Passed" + color.END)
        ...
        errMessage = title + color.YELLOW + "Error: Searching Charlie does not return HashItem('Charlie', 91)" + color.END
        charlie = table.search("Charlie")
        assert hasattr(charlie, "key") and hasattr(charlie, "value") and  charlie.key == "Charlie" and charlie.value == 91, errMessage
        print(title + color.GREEN + "Passed" + color.END)
        return True
